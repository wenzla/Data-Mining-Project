...
'''
So far this parser creates a csv file called topics.csv.
...
Comma-separated-value: each csv is the topics for each article; if blank, no topics were listed;
multiple topics separated by whitespace.
...
def parseFile(filename):
	...
	for x in range(0,len(titles)):
		for title in titles[x]:
			count = str(len(title.split()))
			charcount = str(sum(c != ' ' for c in title))
			outputstring = outputstring + count +" "+ charcount
		#End For
